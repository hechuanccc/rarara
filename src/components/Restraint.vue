  ...
  <div class="restraint-dialog">
    <div v-if="restraint.dialogVisible && restraint.content === 'all'">
      <el-menu :default-active="'0'" class="m-b-xlg" mode="horizontal">
        <el-menu-item v-for="(tab, index) in restraint.tabs" :key="index" :index="index + ''"
          @click.native="switchBlockTab(index)">{{tab.display}}</el-menu-item>
      </el-menu>
      <el-table :data="restraint.nowTab === '1' ? manage.bannedUsers : manage.blockedUsers"
        style="width: 100%">
        <el-table-column
          prop="username"
          label="帐号"
        ...
        <el-table-column v-if="restraint.nowTab === '1'"
          prop="banned_time"
          label="时间(mins)"
          width="160">
        </el-table-column>
        <el-table-column label="操作" :width="restraint.nowTab === '0' ? 215 : 160">
          <template slot-scope="scope">
    ...
    <div class="member-area" v-if="restraint.dialogVisible && restraint.content === 'single'">
      <div class="information text-center" v-if="restraint.user">
        ...
        <div class="m-t">
          ...
          <div>
            <span v-for="(role, index) in restraint.user.roles" :key="index">({{role.display_name}})</span>
          </div>
        </div>
      </div>
      <div slot="footer"
...
export default {
  ...
  created () {
    this.restraint.content === 'single' ? this.getUser() : this.getRoomsUsers()
  }
}
</script>

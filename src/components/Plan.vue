<template>
  <div class="plan">
    <el-tabs v-model="activeGame" v-loading="!gameList.length" @tab-click="changeGame" type="card">
      <el-tab-pane :label="g.display_name" :name="`${i}`" :key="g.id" v-loading="loading" element-loading-background="rgba(0, 0, 0, 0)"
        v-for="(g, i) in gameList">
        <div class="full-height">
          <div class="scheme">
            <ul>
              <li :key="s.id" v-for="(s, i) in schemeList">
                <el-button type="text" :class="{'is-active': i === activeScheme}" @click="changeScheme(i)">
                  {{ s.display_name }}
                </el-button>
              </li>
            </ul>
...
<script>
  ...
  export default {
    ...
    methods: {
      scrollToEnd () {
        this.$nextTick(() => {
          const view = this.$refs.view[this.activeGame]
